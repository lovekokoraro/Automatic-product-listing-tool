...
class RMTScraperGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Maclub自動出品システム")
    ...
    def scrape_gameclub(self):
        ...
        try:
            ...
            with sync_playwright() as p:
                ...
                self.log("https://maclub.jp/signin に移動しています...")
                page.goto("https://maclub.jp/signin")
                ...
                for id, row in enumerate(all_rows):
                    ...
                    product_formtitle = row[0] #案件カテゴリー
                    product_category = row[1] #種別
                    product_title = row[2] #案件タイトル
                    product_description = row[3] #案件説明
                    product_billing = row[4] #収益/月
                    product_pv = row[5] #PV数/月
                    product_uu = row[6] #UU数/月  
                    product_sell = row[7] #販売実績
                    product_inventory = row[8] #商品数
                    product_message = row[9] #買い主へ初回自動表示するメッセージ
                    product_notifi = row[10] #案件の登録を通知
                    product_price = row[11] #売却価格
                    product_discussion = row[12] #金額の相談
                    product_password = row[13] #暗証番号を設定する
                    ...
                    try:
                        page.goto("https://maclub.jp/mypage/products/add")
                        page.wait_for_load_state("load")
                        if is_first:
                            is_first = False
                        else:
                        ...
                        #PV数/月
                        self.log(f"product_pv: {product_pv}")
                        page.locator('input[name="subcategory_unique_property_2_value"]').fill(product_pv)
                        
                        #UU数/月
                        self.log(f"product_uu: {product_uu}")
                        page.locator('input[name="subcategory_unique_property_3_value"]').fill(product_uu)
                        
                        #販売実績
                        self.log(f"product_sell: {product_sell}")
                        page.locator('input[name="subcategory_unique_property_4_value"]').fill(product_sell)
                        
                        #商品数
                        self.log(f"product_inventory: {product_inventory}")
                        page.locator('input[name="subcategory_unique_property_5_value"]').fill(product_inventory)
                        
                        #買い主へ初回自動表示するメッセージ
                        self.log(f"product_message: {product_message}")
                        page.locator('#firstchat').fill(product_message)
                        
                        #案件の登録を通知
                        self.log(f"product_notifi: {product_notifi}")
                        page.locator('input[name="notify_user_id"]').fill(product_notifi)
